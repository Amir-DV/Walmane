...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        if (!['class_select'].includes(interaction.customId)) return
        ...
        const className = interaction.values[0] // Get the selected class name

        const options = ClassSpecRoles[className].map(({ spec, emoji }) => ({
            label: `${spec}`,
            value: `${spec}`,
            emoji: `${emoji}`,
        }))
        ...
        const selectRow1 = new ActionRowBuilder().setComponents(classSelectMenu)
        const selectRow2 = new ActionRowBuilder().setComponents(specSelectMenu)
        await interaction.editReply({
            embeds: [ephemeralEmbed],
            components: [selectRow1, selectRow2], // Update message with new specSelectMenu
        })
    },
}
